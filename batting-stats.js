...
pipeline.on('data', (gameDataUpdate) => {
  ...
  currGameStates.forEach((gameState) => {
    ...
    const currBatter = gameState.topOfInning ? gameState.awayBatter : gameState.homeBatter;
    const currBatterName = gameState.topOfInning ? gameState.awayBatterName : gameState.homeBatterName;
    const prevBatter =
      prevGameState && (prevGameState.topOfInning ? prevGameState.awayBatter : prevGameState.homeBatter);
    const prevBatterName =
      prevGameState && (prevGameState.topOfInning ? prevGameState.awayBatterName : prevGameState.homeBatterName);

    // Create initial summary objects if batter hasn't been previously seen
    if (currBatter && !Object.prototype.hasOwnProperty.call(batterSummaries, currBatter)) {
    ...
    if (prevBatter && !Object.prototype.hasOwnProperty.call(batterSummaries, prevBatter)) {
    ...
    if (
      ...
      batterSummaries[currBatter].postseasons[gameState.season] = initialBatterStatsObject();
    }
    ...
    if (
      ...
      batterSummaries[prevBatter].postseasons[gameState.season] = initialBatterStatsObject();
    }
    ...
    if (
      ...
      batterSummaries[currBatter].seasons[gameState.season] = initialBatterStatsObject();
    }
    ...
    if (
      ...
      batterSummaries[prevBatter].seasons[gameState.season] = initialBatterStatsObject();
    }
    ...
    if (currBatterSummary && currBatterSummary.team === null) {
      currBatterSummary.team = gameState.topOfInning ? gameState.awayTeam : gameState.homeTeam;
    }

    if (currBatterSummary && currBatterSummary.teamName === null) {
      currBatterSummary.teamName = gameState.topOfInning ? gameState.awayTeamName : gameState.homeTeamName;
    }

    if (prevBatterSummary && prevBatterSummary.team === null) {
      prevBatterSummary.team = prevGameState.topOfInning ? prevGameState.awayTeam : prevGameState.homeTeam;
    }

    if (prevBatterSummary && prevBatterSummary.teamName === null) {
      prevBatterSummary.teamName = prevGameState.topOfInning ? prevGameState.awayTeamName : prevGameState.homeTeamName;
    }
    ...
    if (
      prevBatterSummary &&
      gameState.lastUpdate.match(/(hits a|hit into|fielder's choice|strikes out|struck out|ground out|flyout)/i) !==
        null
    ) {
    ...
    if (prevBatterSummary && gameState.lastUpdate.match(/(home run|scores|grand slam)/i) !== null) {
      prevBatterSummary.runsBattedIn += gameState.halfInningScore - prevGameState.halfInningScore;
    }

    // Increment runs scored for home runs
    if (prevBatterSummary && gameState.lastUpdate.match(/(home run|grand slam)/i) !== null) {
    ...
    if (prevGameState && gameState.lastUpdate.match(/\D scores/i) !== null) {
      ...
      if (batterSummaries.hasOwnProperty(scoringRunnerId)) {
        if (gameState.isPostseason) {
          batterSummaries[scoringRunnerId].postseasons[gameState.season].runsScored += 1;
        } else {
          batterSummaries[scoringRunnerId].seasons[gameState.season].runsScored += 1;
        }
      }
    }
    ...
    if (prevGameState && numberOfRunsScoredMatch !== null) {
      ...
      let prevBasesRunners = prevGameState.baseRunners.slice();

      for (let i = 0; i <= runsScored; i++) {
        let scoringRunnerId = prevBasesRunners.shift();
    ...
    if (prevBatterSummary && gameState.lastUpdate.match(/(hits a double)/i) !== null) {
      prevBatterSummary.doublesHit += 1;
    }

    // Increment triples hit
    if (prevBatterSummary && gameState.lastUpdate.match(/(hits a triple)/i) !== null) {
    ...
    if (prevBatterSummary && gameState.lastUpdate.match(/(strikes out|struck out)/i) !== null) {
      prevBatterSummary.strikeouts += 1;
    }

    // Increment ground into double plays
    if (prevBatterSummary && gameState.lastUpdate.match(/hit into a double play/i) !== null) {
      prevBatterSummary.groundIntoDoublePlays += 1;
    }

    // Increment sacrifice bunts/hits
    if (prevBatterSummary && gameState.lastUpdate.match(/scores on the sacrifice/i) !== null) {
      prevBatterSummary.sacrificeBunts += 1;
    }

    // Increment sacrifice flies
    if (prevBatterSummary && gameState.lastUpdate.match(/sacrifice fly/i) !== null) {
    ...
    if (prevGameState && stolenBaseMatch !== null) {
      let prevBasesOccupied = prevGameState.basesOccupied.slice();
      let basesOccupied = gameState.basesOccupied.slice();

      // let stolenBases = [];

      // Identify all bases that were stolen
      for (let base of prevBasesOccupied) {
        if (base === 2) {
        ...
        if (prevBasesOccupied[base] !== basesOccupied[base]) {
          ...
          if (batterSummaries.hasOwnProperty(prevGameState.baseRunners[base])) {
            if (gameState.isPostseason) {
              batterSummaries[prevGameState.baseRunners[base]].postseasons[gameState.season].stolenBases += 1;
            } else {
              batterSummaries[prevGameState.baseRunners[base]].seasons[gameState.season].stolenBases += 1;
            }
          }
        }
      }
    ...
    const caughtStealingMatch = gameState.lastUpdate.match(/([\w].*?) gets caught stealing/i);
    if (prevGameState && caughtStealingMatch !== null) {
      let caughtStealingName = caughtStealingMatch[1];

      // Identify runner caught stolen
      for (const base of prevGameState.basesOccupied) {
        if (prevGameState.basesOccupied[base] !== gameState.basesOccupied[base]) {
          const runnerId = prevGameState.baseRunners[base];

          if (batterSummaries[runnerId].name === caughtStealingName) {
            let runnerSummary = gameState.isPostseason
    ...
    const incineratedPlayerMatch = gameState.lastUpdate.match(/Rogue Umpire incinerated [\w\s]+ hitter ([\w\s]+)!/i);
...
pipeline.on('end', async () => {
  Object.keys(batterSummaries).forEach((batter) => {
    let careerPostseasonData = batterSummaries[batter].careerPostseason;
    let careerSeasonData = batterSummaries[batter].careerSeason;
    let seasonsData = batterSummaries[batter].seasons;
    let postseasonsData = batterSummaries[batter].postseasons;

    Object.keys(seasonsData).forEach((season) => {
      let seasonStats = batterSummaries[batter].seasons[season];
      ...
      careerSeasonData.groundIntoDoublePlays += seasonStats.groundIntoDoublePlays;
    ...
    Object.keys(postseasonsData).forEach((postseason) => {
      let postseasonStats = batterSummaries[batter].postseasons[postseason];

      // Calculate non-tally based postseason stats
      postseasonStats.battingAverage = calculateBattingAverage(postseasonStats);
      postseasonStats.onBasePercentage = calculateOnBasePercentage(postseasonStats);
      postseasonStats.sluggingPercentage = calculateSluggingPercentage(postseasonStats);
      postseasonStats.totalBases = calculateTotalBases(postseasonStats);
      // OPS must go after OBP and SLG calculations
      postseasonStats.onBasePlusSlugging = calculateOnBasePlusSlugging(postseasonStats);
      ...
      careerPostseasonData.groundIntoDoublePlays += postseasonStats.groundIntoDoublePlays;
    ...
    careerSeasonData.onBasePercentage = calculateOnBasePercentage(careerSeasonData);
    careerSeasonData.sluggingPercentage = calculateSluggingPercentage(careerSeasonData);
    careerSeasonData.totalBases = calculateTotalBases(careerSeasonData);
    // OPS must go after OBP and SLG calculations
    careerSeasonData.onBasePlusSlugging = calculateOnBasePlusSlugging(careerSeasonData);
    ...
    careerPostseasonData.battingAverage = calculateBattingAverage(careerPostseasonData);
    careerPostseasonData.onBasePercentage = calculateOnBasePercentage(careerPostseasonData);
    careerPostseasonData.sluggingPercentage = calculateSluggingPercentage(careerPostseasonData);
    careerPostseasonData.totalBases = calculateTotalBases(careerPostseasonData);
    // OPS must go after OBP and SLG calculations
    careerPostseasonData.onBasePlusSlugging = calculateOnBasePlusSlugging(careerPostseasonData);
  });

  // Output objects to JSON files
  await fs.promises.mkdir('./data/batting', { recursive: true }, (err) => {
    if (err) throw err;
  });
  const batterSummariesWriteStream = fs.createWriteStream('./data/batting/batters.json', {
    flags: 'a'
  });
  batterSummariesWriteStream.write(`${JSON.stringify({ ...batterSummaries }, null, '\t')}\n`);
  batterSummariesWriteStream.end();

  // Output individual batters summaries
  Object.keys(batterSummaries).forEach(async (batter) => {
    await fs.promises.mkdir(`./data/batting/${batterSummaries[batter].slug}`, { recursive: true }, (err) => {
      if (err) throw err;
    });
    ...
    batterSummaryWriteStream.write(`${JSON.stringify({ ...batterSummaries[batter] }, null, '\t')}\n`);
...
function calculateOnBasePercentage(stats) {
  return stats.atBats + stats.basesOnBalls + stats.sacrificeFlies > 0
    ? (stats.hits + stats.basesOnBalls) / (stats.atBats + stats.basesOnBalls + stats.sacrificeFlies)
    : 0;
}
...
function calculateSluggingPercentage(stats) {
  const singlesHit = stats.hits - (stats.doublesHit + stats.triplesHit + stats.homeRunsHit);

  return stats.atBats > 0
    ? (singlesHit + stats.doublesHit * 2 + stats.triplesHit * 3 + stats.homeRunsHit * 4) / stats.atBats
    : 0;
}
...
function calculateTotalBases(stats) {
  ...
  return singlesHit + stats.doublesHit * 2 + stats.triplesHit * 3 + stats.homeRunsHit * 4;
}
...
function createPlayerObject({ initialValues, relativeGameState }) {
  const currTeamId = relativeGameState.topOfInning ? relativeGameState.awayTeam : relativeGameState.homeTeam;
